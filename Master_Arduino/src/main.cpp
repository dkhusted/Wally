...
/*
  I lib/Wally_handsignTest_multipleObjects_inferencing/src/edge-impulse-sdk/classifier/ei_run_dsp.h
    Findes der en funktion :extract_image_features_quantized
    Den 

    Teori for I2C: https://www.robot-electronics.co.uk/i2c-tutorial
...
void setup()
{
    ...
    Serial.println("Press c to generate raw features. \n They will be output to serial monitor.");
}
...
void loop()
{
    ...
    if(Serial.read() =='c') {
        ...
        if (ei_sleep(2000) != EI_IMPULSE_OK) {
            ei_printf("ERR: Failed to wait on signal\n");
        }
        ...
        if (ei_run_impulse_check_canceled() == EI_IMPULSE_CANCELED) {
          ei_printf("ERR: Sampel thread was canceled");
        }
